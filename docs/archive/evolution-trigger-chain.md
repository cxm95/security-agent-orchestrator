# Evolution 触发与调用链 — 开发者参考

> **本文档是 CAO Evolution 系统的端到端调用链参考。**
> 目标读者：需要理解、调试或扩展 evolution 机制的开发者。
...
- Hub（CAO Server，端口 `:9889`）是**同步中心** — 存储分数、比较历史、触发心跳提示
- Agent 收到心跳提示后，自行加载 evo-skill 并执行进化
...
| `feedback_reflect` | `prompts/feedback_reflect.md` | 收到人类反馈 | 内联处理（无 evo-skill） |
...
```
cao-bridge/
├── cao_bridge.py          # 主 Bridge 逻辑（get_task, report_score, create_task 等）
├── cao_bridge_mcp.py      # MCP 集成（14 个工具）
...
|--------|----------|----------------|-------------|
| **MCP Bridge** | `cao_bridge_mcp.py` | Agent 手动 `cao_get_task` → 加载 grader skill | ScoreResponse 中的心跳 → Agent 直接处理 |
...
### 10.2 Bridge 侧工具（`cao_bridge_mcp.py`，14 个工具）
...
| `mcp_server/evolution_tools.py` | Hub 侧 14 工具: `cao_report_score()`, `cao_recall()`, `cao_fetch_document()`, `cao_create_task()` 等 |
| `cao-bridge/cao_bridge.py` | `get_task()` ~行 202, `create_task()` ~行 184, `report_score()` |
| `cao-bridge/cao_bridge_mcp.py` | Bridge 侧 14 工具: `cao_get_task`, `cao_create_task`, `cao_recall` 等 |
