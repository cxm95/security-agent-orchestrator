...
### GET /evolution/index
Get the current L1 Knowledge Index content (Insight Index).

The index is generated by the Root Orchestrator from notes in `.cao-evolution/notes/`.

**Response:** Plain text content of `index.md`
```
...
If no index exists yet, returns:
```
# Knowledge Index
...
### POST /evolution/index/rebuild
Trigger a manual rebuild of the L1 Knowledge Index.

The Root Orchestrator will re-read all notes and regenerate `index.md`.

**Response:**
```json
{
  "status": "triggered",
  "message": "Index rebuild message sent to Root Orchestrator"
}
```

If Root Orchestrator is not running:
```json
{
  "status": "skipped",
  "message": "Root Orchestrator not available"
}
```
