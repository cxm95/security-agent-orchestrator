...
├── api/
│   ├── __init__.py
│   ├── main.py                      [~980行] FastAPI 服务 — HTTP 入口 + 基础路由
│   └── evolution_routes.py          [~761行] 进化 API 端点（任务/分数/排行/知识/心跳/recall）
├── mcp_server/
...
│   └── clother_minimax_cn.py        Clother MiniMax CN（继承 claude_code）
├── clients/
...
│  进化层 (Evolution)    │                                  │
│  ┌─────────────────────┘                                  │
│  │  evolution_routes.py  ◄── 分数/任务/知识 API           │
