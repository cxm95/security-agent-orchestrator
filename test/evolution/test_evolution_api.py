...
class TestTaskEndpoints:
    ...
    def test_get_task_no_grader_skill(self, client):
        """Task without grader_skill should return empty string."""
        client.post("/evolution/tasks", json={"task_id": "no-grader"})
        r = client.get("/evolution/no-grader")
        assert r.status_code == 200
        assert r.json()["grader_skill"] == ""
