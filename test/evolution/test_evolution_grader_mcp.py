...
class TestGraderToAPIFlow:
    ...
    def test_grade_then_report(self):
        """Full flow: create task with grader_skill → get task info → report score → leaderboard."""
        ...
        # 2. Remote agent gets task info (replaces pulling grader code)
        r = client.get("/evolution/sec-audit")
        task_info = r.json()
        assert task_info["grader_skill"] == "security-grader"

        # 3. Agent loads evo-skills/security-grader/SKILL.md, evaluates, gets score
        # (simulated — in real flow the agent's LLM reads SKILL.md and produces score)
        score = 0.7
        ...
        r = client.post("/evolution/sec-audit/scores", json={
            ...
            "score": score,
            "title": "scan round 1",
            "feedback": "graded via security-grader skill",
        })
        data = r.json()
...
class TestReportMCPTools:
    def test_report_tools_registered(self):
        ...
        # fastmcp >= 2.x: get_tools() returns {name: Tool} dict
        tools_map = asyncio.run(mcp.get_tools())
