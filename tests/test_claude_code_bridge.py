...
CLAUDE_DIR = Path(__file__).resolve().parent.parent / "cao-bridge" / "claude-code"
...
class TestCLAUDEmd:
    ...
    EXPECTED_TOOLS = [
        ...
        "cao_report_score", "cao_get_grader", "cao_get_leaderboard",
        "cao_share_note", "cao_share_skill", "cao_search_knowledge",
    ]
...
class TestEvolveCommand:
    ...
    def test_references_evolution_tools(self):
        text = (CLAUDE_DIR / "commands" / "evolve.md").read_text()
        for tool in ["cao_report_score", "cao_share_note", "cao_get_leaderboard"]:
