...
class TestMemoryParser:
    ...
    def parser_module(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "memory_parser",
            str(Path(__file__).parent.parent / "cao-bridge" / "hermes-plugin" / "memory_parser.py"),
        )
...
class TestHermesPlugin:
    ...
    def plugin_module(self):
        ...
        spec = importlib.util.spec_from_file_location(
            "hermes_plugin",
            str(Path(__file__).parent.parent / "cao-bridge" / "hermes-plugin" / "__init__.py"),
            submodule_search_locations=[
                str(Path(__file__).parent.parent / "cao-bridge" / "hermes-plugin"),
            ],
        )
        mod = importlib.util.module_from_spec(spec)
    ...
    def test_push_skills_scans_directory(self, plugin_module, tmp_path):
        """Test _push_skills reads hermes skills dir and calls bridge.share_skill."""
        # Create mock skills
        skill1 = tmp_path / "detect-sqli"
        skill1.mkdir()
        (skill1 / "SKILL.md").write_text("---\nname: detect-sqli\n---\nSQLi detection skill")

        skill2 = tmp_path / "xss-scanner"
        skill2.mkdir()
        ...
        assert count == 2
        assert bridge.share_skill.call_count == 2
        # Verify tags include 'hermes'
        for c in bridge.share_skill.call_args_list:
            assert "hermes" in c.kwargs["tags"]

    def test_push_memory_parses_and_shares(self, plugin_module, tmp_path):
        """Test _push_memory parses MEMORY.md and calls bridge.share_note."""
        mem_file = tmp_path / "MEMORY.md"
        ...
        assert count == 3
        assert bridge.share_note.call_count == 3
        for c in bridge.share_note.call_args_list:
            assert "hermes" in c.kwargs["tags"]
            assert "memory" in c.kwargs["tags"]
    ...
    def test_push_skills_empty_dir(self, plugin_module, tmp_path):
        ...
        with patch.dict("os.environ", {"HERMES_SKILLS_DIR": str(tmp_path)}):
            count = plugin_module._push_skills(bridge)
        assert count == 0
        bridge.share_skill.assert_not_called()
    ...
    def test_push_memory_no_file(self, plugin_module, tmp_path):
        ...
        with patch.dict("os.environ", {"HERMES_MEMORY_PATH": str(tmp_path / "nonexistent.md")}):
            count = plugin_module._push_memory(bridge)
        assert count == 0
        bridge.share_note.assert_not_called()
